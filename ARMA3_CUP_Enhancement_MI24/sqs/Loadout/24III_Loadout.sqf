...
_plane removeMagazinesturret ["CUP_4Rnd_AT9_M",[0]];
...
// ataka stuff
_plane animate ["hideAtaka", 1, true];
_atakaCount = 0;
...
sleep 0.3;
	...
	Switch (_hardpoint5) Do
	{
		...
		case 1:
		{
			_atakaCount = (_atakaCount + 1);
			_plane animate ["hideAtaka", 0, true];
			_plane addmagazine "CUP_4Rnd_AT9_M";
		};
	};
...
sleep 0.3;
	...
	// base proxies then load ataka proxy mags - detect ataka from script and run anim for pods
	_plane addmagazine "FIR_Empty_1rnd_M"; 
	_plane addmagazine "FIR_Empty_1rnd_M"; 
	_plane addmagazine "FIR_Empty_1rnd_M"; 
	_plane addmagazine "FIR_Empty_1rnd_M"; 
...
else 
{
	Switch (_preset) do
	{
		...
		case 1:
		{
			if (isNil {profileNamespace getVariable "24III_Custom_slot1"}) then
			{
			...
			else
			{
				...
				_loadcustom1 = ["custom1"]execVM "ARMA3_CUP_Enhancement_MI24\sqs\Loadout\24III_Custom_Apply.sqf";
			};		
		};

		case 2:
		{
			if (isNil {profileNamespace getVariable "24III_Custom_slot2"}) then
			{
			...
			{
				titleText ["Found custom loadout data. Loading..", "PLAIN DOWN",0.5];
				_loadcustom1 = ["custom2"]execVM "ARMA3_CUP_Enhancement_MI24\sqs\Loadout\24III_Custom_Apply.sqf";
			};			
		};

		case 3:
		{
			if (isNil {profileNamespace getVariable "24III_Custom_slot3"}) then
			{
			...
			{
				titleText ["Found custom loadout data. Loading..", "PLAIN DOWN",0.5];
				_loadcustom1 = ["custom3"]execVM "ARMA3_CUP_Enhancement_MI24\sqs\Loadout\24III_Custom_Apply.sqf";
			};	
		};
	};
};
...
_atakacount = {_x == "CUP_4Rnd_AT9_M"} count magazines _plane;

if (_atakacount >= 1) then {_plane addweaponturret ["CUP_Vmlauncher_AT9_veh",[0]];};
if (_s8count >= 1) then 
{
