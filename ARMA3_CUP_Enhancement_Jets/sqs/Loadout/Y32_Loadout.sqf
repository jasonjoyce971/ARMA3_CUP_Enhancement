...
_plane removeweaponturret ["JAS_BIS_weapon_AGM_KH25MPU",[0]];
...
else 
{
	Switch (_preset) do
	{
		...
		case 1:
		{
			if (isNil {profileNamespace getVariable "Y32_Custom_slot1"}) then
			{
			...
			else
			{
				...
				_loadcustom1 = ["custom1"]execVM "ARMA3_CUP_Enhancement_Jets\sqs\Loadout\Y32_Custom_Apply.sqf";
			};		
		};

		case 2:
		{
			if (isNil {profileNamespace getVariable "Y32_Custom_slot2"}) then
			{
			...
			{
				titleText ["found custom loadout data. loading..", "PLAIN DOWN",0.5];
				_loadcustom1 = ["custom2"]execVM "ARMA3_CUP_Enhancement_Jets\sqs\Loadout\Y32_Custom_Apply.sqf";
			};			
		};

		case 3:
		{
			if (isNil {profileNamespace getVariable "Y32_Custom_slot3"}) then
			{
			...
			{
				titleText ["found custom loadout data. loading..", "PLAIN DOWN",0.5];
				_loadcustom1 = ["custom3"]execVM "ARMA3_CUP_Enhancement_Jets\sqs\Loadout\Y32_Custom_Apply.sqf";
			};	
		};
	};
};
