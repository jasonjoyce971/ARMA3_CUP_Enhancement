...
if (_activated) then {
		...
		_hp11 = _logic getVariable "HP11";
		_skin = _logic getVariable "skin";
		
		{
			_plane = vehicle _x;
			if (_plane iskindof "JAS_RHS_B_A10A_USA") then
			{
				[_plane,_hp1,_hp2,_hp3,_hp4,_hp5,_hp6,_hp7,_hp8,_hp9,_hp10,_hp11,_skin] spawn JAS_fnc_rhs_a10_loadout_set;
			}
			else 
			{
				hint "module is must sync with A-10!";
			};
		} foreach _units;
